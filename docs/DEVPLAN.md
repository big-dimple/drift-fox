...
- [ ] 北极狐网格：程序化 LowPoly 分件（three.js 优先——四肢/尾巴绕各自枢轴运动，免导出往返；不够再升级 Blender）。雪白主色、深色耳尖爪、大尾巴剪影，以原画为准；代码驱动奔跑/漂移姿态；
- [ ] 自动全速奔跑 + Shift 抓地漂移（抓地 = 向心 G 与角速度权威提升、留下爪痕 decal + 冷气粒子）+ 松开爆发蹬冰；
- [ ] 寒气值：漂移积蓄、满格待发（HUD 最小槽先做占位）；
- [ ] 平地试滑区：一段直道 + 两个发卡弯。
- **验收**：30 秒试玩想继续；爪痕/冷气在雪面可读；build + smoke 绿；双尺寸截图。
