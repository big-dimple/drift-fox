...
- [x] **两层毛发壳 + 剪影毛发**：底皮同拓扑外扩 +2.2cm/+3.8cm 双层 shell（共享骨骼权重，toon shader 按管状 UV 发丝 alpha 裁剪，内密外稀）；底皮轮廓 per-ring `spike` + per-sector `jag` 细锯齿；尾巴按原画重建为羽毛掸子形（粗基-蓬中-收束-炸尖）+ 胸前鬃毛/颊毛/颈后毛；鼻尖/耳尖/爪尖深色修复（前向环材质规则）+ 眼睛贴面 + 毛壳开口封盖。
