...
| `src/core/timeOfDay.ts` `nightPalette.ts` `palette.ts` | 极光夜 | 变种为「雪夜极光」色彩剧本 |
...
- 移植渲染五件套 + loop/input/stage，空场景跑出极光雪夜天空；
...
- 松树林回转门、扬尘尾迹、冰面裂纹 decal（复用 M1 decal 池）、雪夜极光色彩剧本调优。
