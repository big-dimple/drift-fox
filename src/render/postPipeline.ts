...
/**
 * Minimal presentation state the composite reads each frame. M0 has no fox
 * gameplay state yet; M1's runner state implements this shape.
 */
export interface PostFxState {
  boosting: boolean;
  /** 'surface' while grounded; anything else reads as airborne/leaping. */
  flightPhase: string;
  flightThrust: number;
...
/** Minimal phase view; the composite only needs to know the defeat state. */
export type PostFxPhase = 'running' | 'defeated';
...
export interface PostPipeline {
  ...
  update(dt: number, t: number, state: PostFxState, phase: PostFxPhase): void;
...
export function createPostPipeline(
  ...
  scene: THREE.Scene,
  camera: THREE.Camera,
  prePass: PrePassLike | null,
  quality: RenderQualityProfile,
  layers: PostPipelineLayers = {},
): PostPipeline {
  const guideLayer = layers.guideLayer ?? 0;
  const energyLayer = layers.energyLayer ?? (guideLayer + 2);
  // Preserve the camera's normal mask (ink, energy, world) and add exactly
  // one private guide layer for this view.
  ...
  // Without a prepass (M0 empty scene) the ink mask reads as "no ink solids".
  const emptyInk = new THREE.DataTexture(new Uint8Array(4), 1, 1);
  emptyInk.needsUpdate = true;
  ...
  const finalMaterial = new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      tInk: { value: prePass ? prePass.normalTexture : emptyInk },
  ...
  // The Sobel ink pass only exists when an ink prepass feeds it; M0 renders
  // an empty scene with no ink solids, so the beauty pass goes straight to
  // the composite.
  if (prePass) composer.addPass(createEdgePass(prePass, camera));
